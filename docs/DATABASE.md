...
| `role` | VARCHAR(10) | CHECK (role IN ('user','admin')) | Access level |
...
### 5. `expenses`
Actual financial transactions for a trip.
| Column | Type | Constraints | Description |
| :--- | :--- | :--- | :--- |
| `id` | UUID | PK | |
...
| `type` | VARCHAR(20) | | system, trip, community |
...
*   **Integrity**: `ON DELETE CASCADE` is enforced throughout to prevent orphaned records in child tables when a parent record is deleted.

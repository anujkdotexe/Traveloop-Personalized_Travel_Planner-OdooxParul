...
export default function SharedView() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-page)' }}>
      ...
      <div style={{ height: 420, backgroundImage: 'url(https://images.unsplash.com/photo-1499856871958-5b9627545d1a?auto=format&fit=crop&w=1400&q=80)', backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'relative', textAlign: 'center', color: '#fff', padding: '0 2rem' }}>
          <h1 style={{ color: '#fff', fontSize: 'clamp(2rem,5vw,3.5rem)', marginBottom: '1rem' }}>Summer in Europe</h1>
          <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '1.15rem' }}>14 days across 3 iconic cities &mdash; Shared by Anuj Kondawar</p>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', marginTop: '1.25rem', fontSize: '0.9rem', color: 'rgba(255,255,255,0.75)' }}>
            <span className="flex items-center gap-xs"><CalIcon /> Jun 12 - Jun 25, 2024</span>
            <span className="flex items-center gap-xs"><PinIcon /> 3 Cities</span>
          </div>
        </div>
      </div>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: 'var(--space-xl) 5%' }}>
        <div style={{ marginBottom: 'var(--space-xl)' }}>
          ...
          <div className="flex flex-wrap gap-sm">
            {STOPS.map(s => (
              <span key={s.city} className="flex items-center gap-xs" style={{ padding: '8px 18px', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-full)', fontWeight: 600, fontSize: '0.9rem', boxShadow: 'var(--shadow-sm)' }}>
                <span style={{ color: 'var(--primary)' }}><PinIcon /></span>{s.city}
              </span>
            ))}
          </div>
        </div>
        <div>
          ...
          <div style={{ borderLeft: '2px solid var(--border)', paddingLeft: '2rem' }}>
            {STOPS.map(stop => (
              <div key={stop.city} style={{ marginBottom: '2.5rem', position: 'relative' }}>
                ...
                <h4 style={{ fontSize: '1.15rem', marginBottom: 4 }}>{stop.city}, {stop.country}</h4>
                <p style={{ fontSize: '0.85rem', marginBottom: '1rem' }}>{stop.days}</p>
                <div className="card">
                  <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                    {stop.activities.map(act => (
                      <li key={act} className="flex items-center gap-sm" style={{ fontSize: '0.9rem' }}>
                        <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--primary)', flexShrink: 0 }} />{act}
                      </li>
                    ))}
                  </ul>
                </div>

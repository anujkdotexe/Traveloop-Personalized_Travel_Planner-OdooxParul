...
const recentTrips = [
  { id: 1, name: 'Summer in Europe', dates: 'Jun 12 - Jun 25, 2024', cities: 3, cover: 'https://images.unsplash.com/photo-1499856871958-5b9627545d1a?auto=format&fit=crop&w=600&q=80', status: 'ongoing' },
  { id: 2, name: 'Japan Adventure', dates: 'Sep 10 - Sep 24, 2024', cities: 4, cover: 'https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?auto=format&fit=crop&w=600&q=80', status: 'upcoming' },
];
...
export default function Dashboard() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setShowCreate(false);
      navigate(`/itinerary/${data.data?.id || 1}`);
    } catch (err) {
      ...
      <div className="page-container">
        ...
        <div style={{ height: 280, borderRadius: 'var(--radius-lg)', overflow: 'hidden', background: 'linear-gradient(135deg,var(--primary) 0%,#8b5cf6 50%,var(--secondary) 100%)', position: 'relative', marginBottom: 'var(--space-xl)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ position: 'relative', maxWidth: 520, width: '90%' }}>
            <svg className="icon" style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <input className="input-field" placeholder="Search cities, activities..." style={{ borderRadius: 100, paddingLeft: 48, border: 'none', boxShadow: '0 10px 30px rgba(0,0,0,0.15)' }} />
          </div>
        </div>

        <div className="flex justify-between items-center mb-lg">
          <div><h2>Your Dashboard</h2><p>Manage trips, explore destinations, and stay organized.</p></div>
          <button className="btn btn-primary" onClick={() => setShowCreate(true)}><PlusIcon /> Plan New Trip</button>
        </div>
          ...
          <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: '1.5rem' }}>
            {recentTrips.map(trip => (
              <div key={trip.id} className="card card-hover" style={{ padding: 0, overflow: 'hidden' }}>
                <div style={{ height: 180, backgroundImage: `url(${trip.cover})`, backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative' }}>
                  <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(0,0,0,0.6),transparent)' }} />
                  <div style={{ position: 'absolute', bottom: '1rem', left: '1rem' }}>
                    <span className={`badge badge-${trip.status}`} style={{ marginBottom: 4, display: 'block', width: 'fit-content' }}>{trip.status}</span>
                    <h4 style={{ color: '#fff', fontSize: '1.05rem' }}>{trip.name}</h4>
                  </div>
                </div>
                <div style={{ padding: '1.25rem' }}>
                  <div className="flex items-center gap-md" style={{ fontSize: '0.83rem', color: 'var(--text-muted)', marginBottom: '1.25rem' }}>
                    <span className="flex items-center gap-xs"><CalendarIcon /> {trip.dates}</span>
                    <span className="flex items-center gap-xs"><MapPinIcon /> {trip.cities} cities</span>
                  </div>
            ...
            <button onClick={() => setShowCreate(true)} className="card flex items-center justify-center" style={{ border: '2px dashed var(--border)', background: 'transparent', cursor: 'pointer', minHeight: 280 }}>
        ...
        <section>
          <div className="flex justify-between items-center mb-md">
            ...
            <Link to="/city-search" className="flex items-center gap-xs" style={{ color: 'var(--primary)', fontWeight: 600, fontSize: '0.9rem' }}>Explore All <ArrowRightIcon /></Link>
          </div>
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Plan New Trip">
        <form onSubmit={handleCreate} noValidate>
          ...
          <div className="input-group">
            ...
            <textarea id="description" className="input-field" rows={3} style={{ resize: 'none' }} placeholder="What is this trip about?" value={form.description} onChange={setF('description')} />
          </div>
